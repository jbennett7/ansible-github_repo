...
class GithubRepository(object):
    def __init__(self, token, org, repo):
        ...
        self._org_create_repo_url = "https://api.github.com/orgs/{}/repos".format(org)
        self._user_create_repo_url = "https://api.github.com/user/repos"
        self._repo = repo
        self._org = org
    ...
    def create_repository(self):
        ...
        if r.status_code != 200:
            ...
            if r.status_code != 201:
                r = requests.post(self._user_create_repo_url,data=json.dumps(dict(name=self._repo)),headers=self._header)
    ...
    def sync_config(self):
        ...
        r = requests.post(self._url,json.dumps(self.get_config()),headers=self._header)
        return r, self._url, self.get_config()
  
    def get_labels(self):
        label_list = requests.get(self._labels_url,headers=self._header).json()
        return label_list

    def labels_equal(self, label_list):
        current_label_list = [{'name': d['name'],'color': d['color']} for d in self.get_labels()]
    ...
    def set_labels(self, label_list):
        ...
        for label in label_list:
            if not any(d['name'] == label['name'] for d in current_label_list):
                r = requests.post(self._labels_url,headers=self._header,data=json.dumps(label))
...
def github_repository_present(params):
    ...
    if status == 201:
        ...
        is_error = True
        message['msg']['create_repo_error'] = { 'status': status, 'message': results }
    ...
    if label_list != current_label_list:
        deleted_labels, added_labels, changed_labels = gh.set_labels(label_list)
        has_changed = True
        message['msg']['labels_changes'] = { 'deleted_labels': deleted_labels, 'added_labels': added_labels, 'changed_labels': changed_labels }
    else:
